...
interface SavedReport {
  ...
  prompt: string;
  sql: string | null;
  reportData: { title?: string; summary?: string; columns?: any[] };
...
export default function SavedReportsView() {
  ...
  const fetchReports = useCallback(async () => {
    setLoading(true);
    try {
      let url = "/api/saved-reports?view=";
      if (activeView === "folder" && activeFolderId) {
        url = `/api/saved-reports?view=all&folderId=${activeFolderId}`;
      } else {
        url += activeView;
      }
  ...
  const handleDeleteReport = async (id: string) => {
    try {
      const res = await fetch(`/api/saved-reports?id=${id}`, { method: "DELETE" });
      if (res.ok) {
        fetchReports();
      }
    } catch (e) {
  ...
  const handleTogglePublic = async (report: SavedReport) => {
    try {
  ...
  const handleMoveToFolder = async () => {
    ...
    try {
      const res = await fetch(`/api/saved-reports/${moveReportId}`, {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
      ...
      console.error("Failed to move report:", e);
    }
  };
  ...
  const handleDeleteFolder = async (id: string) => {
    try {
      const res = await fetch(`/api/report-folders?id=${id}`, { method: "DELETE" });
      if (res.ok) {
  ...
  const handleReportClick = (report: SavedReport) => {
    // Open Report Builder with prompt pre-filled via query param
    openTab("Report Builder", `/ai-reports?prompt=${encodeURIComponent(report.prompt)}`);
  };
  ...
  const navItems: { view: ViewType; label: string; icon: typeof Clock; folderId?: string }[] = [
    { view: "recent", label: "Recent", icon: Clock },
    { view: "my", label: "Created by Me", icon: User },
    { view: "public", label: "Public Reports", icon: Globe },
    { view: "all", label: "All Reports", icon: List },
  ];
    ...
    <div className="flex-1 h-full flex flex-col bg-[#c0c0c0] overflow-hidden" style={{ fontFamily: "Segoe UI, Tahoma, sans-serif" }}>
      ...
      <div className="bg-[#f0f0f0] border-b border-[#808080] px-4 py-2 flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <span className="text-[11px] text-[#666]">({filteredReports.length})</span>
        </div>
        <div className="flex items-center gap-2">
          {/* Search */}
          ...
          <button
            onClick={() => openTab("Report Builder", "/ai-reports")}
      ...
      <div className="flex-1 flex overflow-hidden">
          ...
          {folders.map((folder) => {
            ...
            return (
              ...
              >
                ...
                <span className="text-[9px] text-[#999]">{folder._count.reports}</span>
                <button
                  onClick={(e) => {
                    e.stopPropagation();
        ...
        <div className="flex-1 flex flex-col overflow-hidden">
          ...
          <div className="flex bg-[#f0f0f0] border-b border-[#999] text-[11px] font-semibold text-[#333]">
            ...
            <div className="flex-[1] px-2 py-1 border-r border-[#ccc]">Created On</div>
          ...
          <div className="flex-1 overflow-y-auto bg-white">
            {loading ? (
              <div className="flex items-center justify-center py-12">
                ...
                <span className="ml-2 text-[12px] text-[#666]">Loading reports...</span>
              </div>
            ) : filteredReports.length === 0 ? (
              <div className="flex flex-col items-center justify-center py-12 text-[#999]">
                <FileText className="w-8 h-8 mb-2" />
                <p className="text-[12px]">No saved reports yet</p>
                <p className="text-[10px] mt-1">Generate a report in Report Builder and save it</p>
              </div>
            ) : (
              filteredReports.map((report) => (
                ...
                >
                  <div className="flex-[3] px-2 py-1.5 border-r border-[#f0f0f0]">
                    ...
                    {report.description && (
                      <p className="text-[10px] text-[#999] truncate mt-0.5">{report.description}</p>
                    )}
                  </div>
                  ...
                  <div className="w-[100px] px-2 py-1.5 flex items-center justify-center gap-1">
                    ...
                    <button
                      onClick={() => {
                        ...
                        setMoveFolderId(report.folderId);
                      }}
                      className="p-1 hover:bg-[#ddd] rounded"
        ...
        <div className="fixed inset-0 bg-black/30 flex items-center justify-center z-50">
          <div className="bg-[#ece9d8] border-2 border-[#0055e5] shadow-lg w-[320px]" style={{ fontFamily: "Segoe UI, Tahoma, sans-serif" }}>
            <div className="bg-gradient-to-r from-[#0058e6] to-[#3a8cff] px-3 py-1.5 flex items-center justify-between">
              ...
              <button onClick={() => setMoveReportId(null)} className="text-white hover:bg-white/20 p-0.5 rounded">
                <X className="w-3.5 h-3.5" />
              </button>
            </div>
            <div className="p-4">
